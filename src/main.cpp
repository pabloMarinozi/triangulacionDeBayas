...
int main(int argc, char **argv) {
    ...
    if(mpInputReader->error) return -1;
    ...
    Initializer* mpInitializer =  new Initializer(mK, kpsUn1, kpsUn2, 1.0,100000);
	if(mpInitializer->Initialize(mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated)){
		// Set KeyFrame Poses
	...
	float scaleFactor = mpMapManager->GetScaleFactor(distancia_calibracion,mpInputReader);
	if(scaleFactor>0) mpMapManager->ScaleMap(scaleFactor);
	...
	for (auto i : kfRestantes) {
		...
		scaleFactor = mpMapManager->GetScaleFactor(distancia_calibracion,mpInputReader);
		if(scaleFactor>0) mpMapManager->ScaleMap(scaleFactor);
...
//		cout<<mpMapManager->GetDistanceCal1Cal2(cal_1,cal_2)<<endl;
	...
	for (int i = 0; i < numFrames; ++i) {
		vector<cv::Point2f> reprojections = mpMapManager->ReproyectAllMapPointsOnKeyFrame(i);
		vector<cv::Point2f> points = mpMapManager->CreatePointsOnNormalPlane(i, 1);
		...
		for (int j = 0; j < kps.size(); ++j){
			...
			}else errors.push_back(-1);
			cv::circle(img, kps[j], 2, cv::Scalar(255,0,0), 2);
			int track = tracks[j];
			cv::putText(img,to_string(track),kps[j],0,0.5,cv::Scalar(255,0,0));
			cv::circle(img, points[j], 2, cv::Scalar(0,0,255), 2);
			cv::putText(img,to_string(track),points[j],0,0.5,cv::Scalar(0,0,255));
		}
		errors_map[i] = errors;
	...
	mpOutputWriter->guardarResultados(numFrames,errors_map,rep_map,kps,points_map,track_ids,mps,cal_1,cal_2,img_names,radios);
	cout<<"SALIDAS GUARDADAS EN "<<mpOutputWriter->getStrOutputPath()<<endl;
	return 0;
}
